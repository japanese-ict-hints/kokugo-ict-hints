---
title: "俳句をオクリンクプラスで練り、Canvaで作品にして鑑賞し合う"
summary: "季節の言葉から地域の春を俳句にし、オクリンクプラスで見せ合ってブラッシュアップする。仕上がった句はCanvaのAIに読ませて作品カードにし、取り込んで鑑賞会を開く。考える場と仕上げる場を分けている。"
source:
  url: "https://bso.benesse.ne.jp/miraiseed/fansite/info/1190581_1506.html"
  publisher: "ミライシード ファンサイト（ベネッセ）"
  region: "民間"
  published: 2025-10-10
  retrieved: 2026-09-06
  license: link-only
  verified: fetched
tools: [okulink-plus, canva, ai]
school: [elementary, junior]
subjects: [japanese]
domains: [write, language]
scenes: [individual, share, discuss, reflect]
effort: 2
grade: null
unit: ""
ict_use: "俳句をオクリンクプラスで直し合い、Canva連携で作品カードにする"
ict_effect: "考える場と仕上げる場を分けられ、鑑賞会まで一続きになる"
howto:
  - "事前に教育委員会側で、Canva連携の利用設定を済ませてもらう"
  - "オクリンクプラスのCanvaボタンから、各自のCanvaアカウントを接続する"
  - "季節の言葉から俳句を作り、オクリンクプラスで見せ合って直す"
  - "仕上がった句をCanvaで作品カードにする"
  - "カードをオクリンクプラスに取り込み、鑑賞会を開く"
curriculum: []
status: published
added: 2026-09-06
note: "出典が挙げる3つの活用事例のうち、国語のもの。学年の明記はない。この連携は2025年10月10日に追加された機能で、使うには教育委員会側での利用設定が必要（先生のみ／先生と子ども／不可 から選ぶ）。出典はCanva教育版での利用を推奨している。所属校で使えるかは事前に確認すること。"
---

考えを練る場と、仕上げる場を分けているのがこの事例の要点。推敲は互いの句が見える場で行い、作品化は別のツールに任せている。国語としては、清書に凝りすぎて言葉の吟味がおろそかにならないよう、どこまでを推敲の時間とするかを先に決めておきたい。連携の設定が済んでいないと授業当日に動かないので、単元に入る前に一度試しておく必要がある。
