...
出典のある事例だけでは領域や学年に穴が残るため、このサイトが考えた事例も載せている。
`origin: original` を立てると、`source` は `url: ""` / `publisher: "このサイト"` /
`license: original` / `verified: original` でなければビルドが落ちる（出典URLを書けない）。

一覧と詳細に「自作」の印が出て、出典を開くボタンの代わりに「実践の記録ではない」旨の
断り書きが出る。`note` には、どの事例で確かめられた考え方をもとにしたかを書く。
出典のある事例が見つかったら、そちらを優先して置き換える。詳細は CLAUDE.md §4.3。
...
`/units/` は単元別。**出典が単元名を示している事例だけ**が並ぶ。個々の単元名・教材名は
題号なので載せてよい。載せてはいけないのは年間指導計画表そのもの（配当順・時数・
評価規準の対応など、編集に創作性がある表）。詳細は CLAUDE.md §2.1。

単元名は `unit` に書く。**出典が示しているものだけ**で、教材名から学年や発行者を推測しない。
