...
- **「役に立った」ボタン** — 実装済み。押すと端末の中に記録が残り、押済みの表示に変わる。
  **現状は外部へ何も送らない**ので、運営者は数を見られない。集計するなら
  `src/lib/site.ts` の `HELPFUL.countUrl` に、事例の識別子だけを受け取る受け口のURLを書く。
